...
| 2026-08-25 | Deferred (named, not fixed) | — | Same TZ bucketing class exists in 3 pre-existing call sites (months memo, category trend ×2) — the Bars view shares the latent bug west of UTC; skipped: out of this feature's scope, fix as its own change with a shared date-key helper. Demo data cleanup: DONE — all 90 seeded rows deleted via user-scoped API, 0 remaining (2026-08-25); demo token file removed |
