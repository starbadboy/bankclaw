...
| 2026-08-25 | Post-feature: test-suite root cause | Done | The "17 Mongo-timeout failures" were not a missing database: `tests/test_db.py` did `importlib.reload(webapp.db)` inside `patch("webapp.db.MongoClient")`, re-binding the real class and leaving a real `localhost:27017` client in the `_client` singleton. Fixed with `monkeypatch.setattr(db_module, "_client", None)` and no reload. Fast loop: **178 passed in 3.63 s** (was 155 pass / 17 fail in 11 min). `docker-compose.yaml` gained a healthchecked `mongo:7.0` service (published 27017, `mongo_data` volume) and the app now depends on it; verified healthy after pruning dangling Docker images (user-approved). CLAUDE.md lesson rewritten to the real cause. |
| 2026-08-25 | Follow-up: hover tooltip (was `skipped: add when asked`) | Done | User asked after seeing the chart. `NetWorthChart` gained hover state: nearest point → dashed guide, dot, `date · S$ value` tooltip (anchor flips near edges; static last-value label hidden while hovering). `buildHoldingChartData` now carries `date`. Also benefits the net-worth chart (label fallback). Tests: data 24, portfolio 18, all JS green, esbuild clean. Evidence: `panel-hover.png` (`2026-08-11 · S$ 216,462`). Branch `feat/holding-chart-hover`. |
| 2026-08-25 | Hover review (Opus 5) | Done | **Approve**, 4 nits. Applied: dead `\|\| label` fallback removed (both charts supply `date`); dead single-point ternary removed; no-op `pointerEvents` removed. Declined: `fmtDate()` for the tooltip — it parses ISO via `new Date()` (the UTC-midnight pitfall in CLAUDE.md); tooltip keeps `YYYY-MM-DD`. |
