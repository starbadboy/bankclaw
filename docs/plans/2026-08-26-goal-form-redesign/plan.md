...
| 2026-08-26 | Follow-ups (branch feat/goal-form-followups) | Done | render probe, automatic ?v=, CLAUDE.md lessons, repar proposal (07b3584); single-reviewer Block → CRITICAL pre-existing path traversal in serve_spa fixed + no-cache index + ns mtimes + CI node step (2e199c6); verification #1 New problems → NUL-byte 500 regression fixed (9866e53) |
...
- Fix-pass verification #1 on 2e199c6 (Opus 5): **New problems** — `.resolve()` raises `ValueError` on a NUL byte where `.exists()` swallowed it → `GET /%00` 500 (regression). Fixed in 9866e53: `suppress(ValueError)` → SPA fallback; params `a\x00b.js`, `..\x00/.env` added; `/etc/passwd` param made deep enough to actually escape; duplicate import removed. HTTP check: `/%00`, `/..%00/.env`, `/a%00b.js`, `/..%2f.env` → 200 html, no leak; `/app/data.js` → 200 js. Python **209 passed**. Nit recorded, not changed: symlinked assets inside `dashboard/` that point outside are unservable by design.
- Fix-pass verification #2 on 9866e53 (Opus 5, final round): **New problems** — 3/3 addressed; pre-existing sibling raiser: `is_file()` on a >255-char name → `OSError` ENAMETOOLONG → unauthenticated 500. Fixed in the next commit (`suppress(ValueError, OSError)` around resolve + is_file + serve; two over-long params added). **No third verifier round per policy** — evidence is the RED/GREEN test pair and the HTTP probe (over-long, NUL, traversal → 200 shell; `/app/data.js` → 200 js). Nit recorded as follow-up: `ruff`/`isort` are configured but not installed in the venv and CI runs only pytest — the lint config is not enforced (`uv sync --group dev` may need them, or add a lint step).
