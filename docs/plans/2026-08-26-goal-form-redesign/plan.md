...
## Evidence
: (1) `.pf-add-grid` hard-codes 5 columns — modifier with auto-fit added to Task 1 + risk; (2) debt draft needs `baseline` for the helper — stated in Task 1; (3) cache-buster numbering across Tasks 1–2 clarified (v2 then v3). |
...
- GREEN: `GoalPreview` (draft → `computeGoalProgress(goal, goalCtx)`; debt draft takes today's balance as baseline), `GoalForm` on `.pf-add-row pf-goal-form` / `.pf-add-grid` with `<label><span>` captions, `.pf-add-actions`; `GoalsCard`/`PortfolioPage` pass `goalCtx`; CSS `.pf-goal-form` (border + auto-fit grid), `.pf-goal-preview`; cache-busters page-goals v2, page-portfolio v15, styles v13.
