...
from webapp.goal_advisor import build_goal_aggregate, get_suggestions, normalize_suggestions
...
def test_aggregate_contains_only_aggregates_and_never_names_or_tickers():
    ...
    assert agg["existing_goals"] == [
        {"kind": "net_worth", "target": 100000.0, "progress": 0.61},
        {"kind": "debt_payoff", "target": 0, "progress": 0.0},
    ]
    blob = json.dumps(agg)
...
def test_get_suggestions_generates_once_then_serves_cache_and_filters_dismissed(monkeypatch):
    ...
    with (
        ...
        first = get_suggestions("owner@example.com", PORTFOLIO)
    ...
    with patch("webapp.goal_advisor.get_db", return_value=db), patch("webapp.goal_advisor._call_llm") as llm:
        cached = get_suggestions("owner@example.com", PORTFOLIO)
        dismissed = get_suggestions("owner@example.com", PORTFOLIO, dismiss=sid)
...
def test_get_suggestions_force_refresh_regenerates_and_requires_key(monkeypatch):
    ...
    with patch("webapp.goal_advisor.get_db", return_value=db):
        with pytest.raises(ValueError, match="DEEPSEEK_API_KEY"):
            get_suggestions("owner@example.com", PORTFOLIO, force_refresh=True)
    ...
    with (
        ...
        fresh = get_suggestions("owner@example.com", PORTFOLIO, force_refresh=True)
...
def test_concurrent_generation_for_one_user_calls_the_model_once(monkeypatch):
    ...
    with (
        ...
        threads = [
            threading.Thread(target=lambda: results.append(get_suggestions("owner@example.com", PORTFOLIO)))
            for _ in range(3)
        ]
        for t in threads:
            t.start()
